<div class="">
    <div class="mt-auto w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
        <nav class="flex gap-x-1 " aria-label="Tabs" role="tablist" aria-orientation="horizontal">
        <button type="button" class="hs-tab-active:font-semibold hs-tab-active:border-[#812755] hs-tab-active:text-[#812755] py-4 px-8 inline-flex items-center gap-x-2 border-b-2 border-transparent text-lg whitespace-nowrap text-gray-500 hover:text-[#812755] focus:outline-none focus:text-[#812755] disabled:opacity-50 disabled:pointer-events-none dark:text-neutral-400 dark:hover:text-[#812755] active" id="tabs-with-icons-item-1" aria-selected="true" data-hs-tab="#tabs-with-icons-1" aria-controls="tabs-with-icons-1" role="tab">
            <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="1.9"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-tools-kitchen-2"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M19 3v12h-5c-.023 -3.681 .184 -7.406 5 -12zm0 12v6h-1v-3m-10 -14v17m-3 -17v3a3 3 0 1 0 6 0v-3" /></svg>
            
            Restaurant
        </button>
        <button type="button" class="hs-tab-active:font-semibold hs-tab-active:border-[#812755] hs-tab-active:text-[#812755] py-4 px-8 inline-flex items-center gap-x-2 border-b-2 border-transparent text-lg whitespace-nowrap text-gray-500 hover:text-[#812755] focus:outline-none focus:text-[#812755] disabled:opacity-50 disabled:pointer-events-none dark:text-neutral-400 dark:hover:text-[#812755]" id="tabs-with-icons-item-2" aria-selected="false" data-hs-tab="#tabs-with-icons-2" aria-controls="tabs-with-icons-2" role="tab">
            <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="1.9"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-glass-cocktail"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M8 21h8" /><path d="M12 15v6" /><path d="M12 5m-7 0a7 2 0 1 0 14 0a7 2 0 1 0 -14 0" /><path d="M5 5v.388c0 .432 .126 .853 .362 1.206l5 7.509c.633 .951 1.88 1.183 2.785 .517c.191 -.141 .358 -.316 .491 -.517l5 -7.509c.236 -.353 .362 -.774 .362 -1.206v-.388" /></svg>
            Bar
        </button>
        <button type="button" class="hs-tab-active:font-semibold hs-tab-active:border-[#812755] hs-tab-active:text-[#812755] py-4 px-8 inline-flex items-center gap-x-2 border-b-2 border-transparent text-lg whitespace-nowrap text-gray-500 hover:text-[#812755] focus:outline-none focus:text-[#812755] disabled:opacity-50 disabled:pointer-events-none dark:text-neutral-400 dark:hover:text-[#812755]" id="tabs-with-icons-item-3" aria-selected="false" data-hs-tab="#tabs-with-icons-3" aria-controls="tabs-with-icons-3" role="tab">
            <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="1.9"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-baguette"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M5.628 11.283l5.644 -5.637c2.665 -2.663 5.924 -3.747 8.663 -1.205l.188 .181a2.987 2.987 0 0 1 0 4.228l-11.287 11.274a3 3 0 0 1 -4.089 .135l-.143 -.135c-2.728 -2.724 -1.704 -6.117 1.024 -8.841z" /><path d="M9.5 7.5l1.5 3.5" /><path d="M6.5 10.5l1.5 3.5" /><path d="M12.5 4.5l1.5 3.5" /></svg>
            Boulangerie
        </button>
        <button type="button" class="hs-tab-active:font-semibold hs-tab-active:border-[#812755] hs-tab-active:text-[#812755] py-4 px-8 inline-flex items-center gap-x-2 border-b-2 border-transparent text-lg whitespace-nowrap text-gray-500 hover:text-[#812755] focus:outline-none focus:text-[#812755] disabled:opacity-50 disabled:pointer-events-none dark:text-neutral-400 dark:hover:text-[#812755]" id="tabs-with-icons-item-4" aria-selected="false" data-hs-tab="#tabs-with-icons-4" aria-controls="tabs-with-icons-3" role="tab">
            <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="1.9"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-bed"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 9m-2 0a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /><path d="M22 17v-3h-20" /><path d="M2 8v9" /><path d="M12 14h10v-2a3 3 0 0 0 -3 -3h-7v5z" /></svg>
            Hotel
        </button>
        <button type="button" class="hs-tab-active:font-semibold hs-tab-active:border-[#812755] hs-tab-active:text-[#812755] py-4 px-8 inline-flex items-center gap-x-2 border-b-2 border-transparent text-lg whitespace-nowrap text-gray-500 hover:text-[#812755] focus:outline-none focus:text-[#812755] disabled:opacity-50 disabled:pointer-events-none dark:text-neutral-400 dark:hover:text-[#812755]" id="tabs-with-icons-item-5" aria-selected="false" data-hs-tab="#tabs-with-icons-5" aria-controls="tabs-with-icons-3" role="tab">
            <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="1.9"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-hanger-2"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 9l-7.971 4.428a2 2 0 0 0 -1.029 1.749v.823a2 2 0 0 0 2 2h1" /><path d="M18 18h1a2 2 0 0 0 2 -2v-.823a2 2 0 0 0 -1.029 -1.749l-7.971 -4.428c-1.457 -.81 -1.993 -2.333 -2 -4a2 2 0 1 1 4 0" /><path d="M6 16m0 2a2 2 0 0 1 2 -2h8a2 2 0 0 1 2 2v1a2 2 0 0 1 -2 2h-8a2 2 0 0 1 -2 -2z" /></svg>
                Pressing
        </button>
        <button type="button" class="hs-tab-active:font-semibold hs-tab-active:border-[#812755] hs-tab-active:text-[#812755] py-4 px-8 inline-flex items-center gap-x-2 border-b-2 border-transparent text-lg whitespace-nowrap text-gray-500 hover:text-[#812755] focus:outline-none focus:text-[#812755] disabled:opacity-50 disabled:pointer-events-none dark:text-neutral-400 dark:hover:text-[#812755]" id="tabs-with-icons-item-6" aria-selected="false" data-hs-tab="#tabs-with-icons-6" aria-controls="tabs-with-icons-3" role="tab">
            <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="1.9"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-reserved-line"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M9 20h6" /><path d="M12 14v6" /><path d="M4 4m0 2a2 2 0 0 1 2 -2h12a2 2 0 0 1 2 2v6a2 2 0 0 1 -2 2h-12a2 2 0 0 1 -2 -2z" /><path d="M9 9h6" /></svg>
                Riad
        </button>
        </nav>
   

    <div class="mt-3">
        <div id="tabs-with-icons-1" class=" bg-[#f8f5f2] rounded-xl" role="tabpanel" aria-labelledby="tabs-with-icons-item-1">
            <!-- Main Content -->
            <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div class="flex flex-col lg:flex-row gap-12">
                    <!-- Left Column -->
                    <div class="lg:w-1/2  ">
                        <div class="relative ">
                            <div class=" h-[500px] relative overflow-hidden rounded-e-3xl rounded-s-3xl">
                                <img src="{{asset('/images/b/b-1.jpg')}}" alt="Restaurateur" class=" object-cover">
                            </div>
                            <div class="absolute bottom-0 w-full p-8 bg-[#812755] rounded-3xl border-2 border-solid border-white">
                                <div class="text-white font-medium mb-2">RESTAURATEUR</div>
                                <h2 class="text-3xl font-bold mb-4 text-white">On te connaît si bien.</h2>
                                <a href="#" class="text-white flex items-center">
                                    Découvrir
                                    <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                                    </svg>
                                </a>
                            </div>
                        </div>
                    </div>

                    <!-- Right Column -->
                    <div class="lg:w-1/2 text-black">
                        <h1 class="text-2xl font-bold mb-4 text-[#812755]">L'application a été pensée par un restaurateur qui connaît tes contraintes !</h1>
                        
                        <p class="text-[#812755] text-lg mb-3">
                            Notre mission ne se limite pas au déploiement d'une simple caisse enregistreuse.
                        </p>

                        <p class="text-gray-700 mb-8">
                            Popina s'efforce de mettre à disposition une application personnalisée entre les mains des restaurateurs qui l'utilisent. Souscris à tes indispensables et ajoute de nombreux services et accessoires, selon tes besoins, quelle que soit la taille de ta surface de restauration.
                        </p>

                        <div class="space-y-2">
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10H9m3 4v-4m0 0l3-3m-3 3l-3-3m12 1V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2z"/>
                                    </svg>
                                </div>
                                <span>Paiement et prise de commande</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                                    </svg>
                                </div>
                                <span>Gestion des stocks avancée</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                    </svg>
                                </div>
                                <span>Ventilation des ventes et export comptable</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                                    </svg>
                                </div>
                                <span>Organisation des pôles de préparation</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7"/>
                                    </svg>
                                </div>
                                <span>Pilotage du programme de fidélité</span>
                            </div>
                        </div>

                            <div class="py-2">
                                <div class="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                                    <a href="#" class="w-full sm:w-auto inline-block bg-[#812755] text-white text-center px-8 py-2 rounded-full text-lg font-bold hover:bg-[#812755] transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#812755] focus:ring-opacity-50">
                                        Visionner une démo
                                    </a>
                                    <a href="#" class="w-full sm:w-auto inline-block bg-transparent border-2 border-[#812755] text-[#812755] text-center px-8 py-2 rounded-full text-lg font-bold hover:bg-[#812755] hover:text-white transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#812755] focus:ring-opacity-50">
                                        Contactez-nous
                                    </a>
                                </div>
                            </div>
                        </div>
                </div>
            </main>
        </div>
        <div id="tabs-with-icons-2" class="hidden bg-[#f8f5f2] rounded-xl" role="tabpanel" aria-labelledby="tabs-with-icons-item-2">
            <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div class="flex flex-col lg:flex-row gap-12">
                    <!-- Left Column -->
                    <div class="lg:w-1/2  ">
                        <div class="relative ">
                            <div class=" h-[500px] relative overflow-hidden rounded-e-3xl rounded-s-3xl">
                                <img src="{{asset('/images/b/bar.jpg')}}" alt="Caisse Tactile pour Bar" class=" object-cover">
                            </div>
                            <div class="absolute bottom-0 w-full p-8 bg-[#812755] rounded-3xl border-2 border-solid border-white">
                                <div class="text-white font-medium mb-2">BAR</div>
                                <h2 class="text-3xl font-bold mb-4 text-white">On te connaît si bien.</h2>
                                <a href="#" class="text-white flex items-center">
                                    Découvrir
                                    <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                                    </svg>
                                </a>
                            </div>
                        </div>
                    </div>

                    <!-- Right Column -->
                    <div class="lg:w-1/2 text-black">
                        <h1 class="text-2xl font-bold mb-4 text-[#812755]">Caisse tactile pour bars et cafés au Maroc</h1>
                        
                        <p class="text-[#812755] text-lg mb-3">
                            Système de caisse enregistreuse horeca avec gestion des paiements TPE
                        </p>

                        <p class="text-gray-700 mb-8">
                            Notre solution de caisse tactile pas cher est idéale pour les bars et cafés. Le logiciel caisse enregistreuse intègre toutes les fonctionnalités essentielles : gestion des stocks, des commandes et système TPE paiement Maroc. Compatible avec les standards horeca, notre caisse automatique Maroc facilite la gestion quotidienne de votre établissement.
                         </p>

                        <div class="space-y-2">
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10H9m3 4v-4m0 0l3-3m-3 3l-3-3m12 1V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2z"/>
                                    </svg>
                                </div>
                                <span>Caisse enregistreuse avec écran tactile haute performance</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                                    </svg>
                                </div>
                                <span>Système de gestion des stocks de boissons intégré</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                    </svg>
                                </div>
                                <span>Bon de caisse et factures personnalisables</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                                    </svg>
                                </div>
                                <span>CSupport technique et formation inclus</span>
                            </div>
                        </div>

                            <div class="py-2">
                                <div class="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                                    <a href="#" class="w-full sm:w-auto inline-block bg-[#812755] text-white text-center px-8 py-2 rounded-full text-lg font-bold hover:bg-[#812755] transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#812755] focus:ring-opacity-50">
                                        Visionner une démo
                                    </a>
                                    <a href="#" class="w-full sm:w-auto inline-block bg-transparent border-2 border-[#812755] text-[#812755] text-center px-8 py-2 rounded-full text-lg font-bold hover:bg-[#812755] hover:text-white transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#812755] focus:ring-opacity-50">
                                        Contactez-nous
                                    </a>
                                </div>
                            </div>
                        </div>
                </div>
            </main>
        </div>
        <div id="tabs-with-icons-3" class="hidden bg-[#f8f5f2] rounded-xl" role="tabpanel" aria-labelledby="tabs-with-icons-item-3">
            <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div class="flex flex-col lg:flex-row gap-12">
                    <!-- Left Column -->
                    <div class="lg:w-1/2  ">
                        <div class="relative ">
                            <div class=" h-[500px] relative overflow-hidden rounded-e-3xl rounded-s-3xl">
                                <img src="{{asset('/images/b/boulangerie.jpg')}}" alt="Restaurateur" class=" object-cover">
                            </div>
                            <div class="absolute bottom-0 w-full p-8 bg-[#812755] rounded-3xl border-2 border-solid border-white">
                                <div class="text-white font-medium mb-2">BOULANGERIE</div>
                                <h2 class="text-3xl font-bold mb-4 text-white">On te connaît si bien.</h2>
                                <a href="#" class="text-white flex items-center">
                                    Découvrir
                                    <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                                    </svg>
                                </a>
                            </div>
                        </div>
                    </div>

                    <!-- Right Column -->
                    <div class="lg:w-1/2 text-black">
                        <h1 class="text-2xl font-bold mb-4 text-[#812755]">Caisse enregistreuse Maroc pour boulangeries et pâtisseries</h1>
                        
                        <p class="text-[#812755] text-lg mb-3">
                            Solution كاشير complète avec gestion des stocks et tiroir monnaie
                        </p>

                        <p class="text-gray-700 mb-8">
                            Découvrez notre système de caisse enregistreuse spécialement conçu pour les boulangeries. Notre logiciel intègre un برنامج complet de gestion avec contrôle caisse enregistreuse, gestion des stocks, et impression automatique des bons de caisse. Prix caisse enregistreuse Maroc adapté à tous les budgets.
                        </p>

                        <div class="space-y-2">
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10H9m3 4v-4m0 0l3-3m-3 3l-3-3m12 1V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2z"/>
                                    </svg>
                                </div>
                                <span>Interface كاشير intuitive en français et arabe</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                                    </svg>
                                </div>
                                <span>Gestion des stocks et des invendus automatisée</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                    </svg>
                                </div>
                                <span>Impression tickets et facturation intégrée</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                                    </svg>
                                </div>
                                <span>Support technique en arabe et français</span>
                            </div>
                        </div>

                            <div class="py-2">
                                <div class="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                                    <a href="#" class="w-full sm:w-auto inline-block bg-[#812755] text-white text-center px-8 py-2 rounded-full text-lg font-bold hover:bg-[#812755] transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#812755] focus:ring-opacity-50">
                                        Visionner une démo
                                    </a>
                                    <a href="#" class="w-full sm:w-auto inline-block bg-transparent border-2 border-[#812755] text-[#812755] text-center px-8 py-2 rounded-full text-lg font-bold hover:bg-[#812755] hover:text-white transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#812755] focus:ring-opacity-50">
                                        Contactez-nous
                                    </a>
                                </div>
                            </div>
                        </div>
                </div>
            </main>
        </div>
        <div id="tabs-with-icons-4" class="hidden bg-[#f8f5f2] rounded-xl" role="tabpanel" aria-labelledby="tabs-with-icons-item-4">
            <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div class="flex flex-col lg:flex-row gap-12">
                    <!-- Left Column -->
                    <div class="lg:w-1/2  ">
                        <div class="relative ">
                            <div class=" h-[500px] relative overflow-hidden rounded-e-3xl rounded-s-3xl">
                                <img src="{{asset('/images/b/b-2.jpg')}}" alt="Restaurateur" class=" object-cover">
                            </div>
                            <div class="absolute bottom-0 w-full p-8 bg-[#812755] rounded-3xl border-2 border-solid border-white">
                                <div class="text-white font-medium mb-2">HOTEL</div>
                                <h2 class="text-3xl font-bold mb-4 text-white">On te connaît si bien.</h2>
                                <a href="#" class="text-white flex items-center">
                                    Découvrir
                                    <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                                    </svg>
                                </a>
                            </div>
                        </div>
                    </div>

                    <!-- Right Column -->
                    <div class="lg:w-1/2 text-black">
                        <h1 class="text-2xl font-bold mb-4 text-[#812755]">Système de caisse tactile pour hôtels au Maroc</h1>
                        
                        <p class="text-[#812755] text-lg mb-3">
                            Logiciel caisse enregistreuse tactile avec gestion hôtelière complète
                        </p>

                        <p class="text-gray-700 mb-8">
                            Notre solution combine caisse enregistreuse et système de gestion hôtelière. Disponible sur Avito Maroc et compatible avec les standards internationaux, notre logiciel caisse enregistreuse offre une gestion complète : réservations, facturation, et TPE paiement intégré.
                        </p>

                        <div class="space-y-2">
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10H9m3 4v-4m0 0l3-3m-3 3l-3-3m12 1V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2z"/>
                                    </svg>
                                </div>
                                <span>Système de caisse manager avec gestion des chambres</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                                    </svg>
                                </div>
                                <span>Module de réservation et planning intégré</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                    </svg>
                                </div>
                                <span>Compatible avec tous les TPE de paiement Maroc</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                                    </svg>
                                </div>
                                <span>Formation et support technique inclus</span>
                            </div>
                        </div>

                            <div class="py-2">
                                <div class="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                                    <a href="#" class="w-full sm:w-auto inline-block bg-[#812755] text-white text-center px-8 py-2 rounded-full text-lg font-bold hover:bg-[#812755] transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#812755] focus:ring-opacity-50">
                                        Visionner une démo
                                    </a>
                                    <a href="#" class="w-full sm:w-auto inline-block bg-transparent border-2 border-[#812755] text-[#812755] text-center px-8 py-2 rounded-full text-lg font-bold hover:bg-[#812755] hover:text-white transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#812755] focus:ring-opacity-50">
                                        Contactez-nous
                                    </a>
                                </div>
                            </div>
                        </div>
                </div>
            </main>
        </div>
        <div id="tabs-with-icons-5" class="hidden bg-[#f8f5f2] rounded-xl" role="tabpanel" aria-labelledby="tabs-with-icons-item-5">
            <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div class="flex flex-col lg:flex-row gap-12">
                    <!-- Left Column -->
                    <div class="lg:w-1/2  ">
                        <div class="relative ">
                            <div class=" h-[500px] relative overflow-hidden rounded-e-3xl rounded-s-3xl">
                                <img src="{{asset('/images/b/pressing.jpg')}}" alt="Restaurateur" class=" object-cover">
                            </div>
                            <div class="absolute bottom-0 w-full p-8 bg-[#812755] rounded-3xl border-2 border-solid border-white">
                                <div class="text-white font-medium mb-2">PRESSING</div>
                                <h2 class="text-3xl font-bold mb-4 text-white">On te connaît si bien.</h2>
                                <a href="#" class="text-white flex items-center">
                                    Découvrir
                                    <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                                    </svg>
                                </a>
                            </div>
                        </div>
                    </div>

                    <!-- Right Column -->
                    <div class="lg:w-1/2 text-black">
                        <h1 class="text-2xl font-bold mb-4 text-[#812755]">Caisse enregistreuse tactile pour pressing et salon de coiffure</h1>
                        
                        <p class="text-[#812755] text-lg mb-3">
                            Solution de caisse tactile adaptée aux services de nettoyage
                        </p>

                        <p class="text-gray-700 mb-8">
                            Spécialement conçue pour les pressings et salons de coiffure, notre caisse tactile pas cher intègre toutes les fonctionnalités essentielles. Avec un système similaire aux modèles Sharp XE A217B, notre solution offre une gestion complète des commandes et du suivi client.
                        </p>

                        <div class="space-y-2">
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10H9m3 4v-4m0 0l3-3m-3 3l-3-3m12 1V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2z"/>
                                    </svg>
                                </div>
                                <span>Interface adaptée aux services de pressing</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                                    </svg>
                                </div>
                                <span>Gestion des commandes et délais</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                    </svg>
                                </div>
                                <span>Système de fidélisation client intégré</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                                    </svg>
                                </div>
                                <span>Bobine pour caisse enregistreuse incluse</span>
                            </div>
                        </div>

                            <div class="py-2">
                                <div class="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                                    <a href="#" class="w-full sm:w-auto inline-block bg-[#812755] text-white text-center px-8 py-2 rounded-full text-lg font-bold hover:bg-[#812755] transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#812755] focus:ring-opacity-50">
                                        Visionner une démo
                                    </a>
                                    <a href="#" class="w-full sm:w-auto inline-block bg-transparent border-2 border-[#812755] text-[#812755] text-center px-8 py-2 rounded-full text-lg font-bold hover:bg-[#812755] hover:text-white transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#812755] focus:ring-opacity-50">
                                        Contactez-nous
                                    </a>
                                </div>
                            </div>
                        </div>
                </div>
            </main>
        </div>
        <div id="tabs-with-icons-6" class="hidden bg-[#f8f5f2] rounded-xl" role="tabpanel" aria-labelledby="tabs-with-icons-item-6">
            <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div class="flex flex-col lg:flex-row gap-12">
                    <!-- Left Column -->
                    <div class="lg:w-1/2  ">
                        <div class="relative ">
                            <div class=" h-[500px] relative overflow-hidden rounded-e-3xl rounded-s-3xl">
                                <img src="{{asset('/images/b/riad.jpg')}}" alt="Restaurateur" class=" object-cover">
                            </div>
                            <div class="absolute bottom-0 w-full p-8 bg-[#812755] rounded-3xl border-2 border-solid border-white">
                                <div class="text-white font-medium mb-2">RIAD</div>
                                <h2 class="text-3xl font-bold mb-4 text-white">On te connaît si bien.</h2>
                                <a href="#" class="text-white flex items-center">
                                    Découvrir
                                    <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                                    </svg>
                                </a>
                            </div>
                        </div>
                    </div>

                    <!-- Right Column -->
                    <div class="lg:w-1/2 text-black">
                        <h1 class="text-2xl font-bold mb-4 text-[#812755]">Caisse enregistreuse automatique pour riads محل</h1>
                        
                        <p class="text-[#812755] text-lg mb-3">
                            Solution complète de gestion pour maisons d'hôtes au Maroc
                        </p>

                        <p class="text-gray-700 mb-8">
                            Notre système caisse enregistreuse est parfaitement adapté aux besoins des riads. Disponible sur Ouedkniss et autres plateformes, notre solution inclut un logiciel caisse enregistreuse crack gratuit avec mise à jour régulière et formation complète.
                        </p>

                        <div class="space-y-2">
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10H9m3 4v-4m0 0l3-3m-3 3l-3-3m12 1V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2z"/>
                                    </svg>
                                </div>
                                <span>Gestion des réservations et des services</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                                    </svg>
                                </div>
                                <span>Système de facturation automatisé</span>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-6 h-6 bg-white/10 rounded flex items-center justify-center">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                                    </svg>
                                </div>
                                <span>Support technique local au Maroc</span>
                            </div>
                        </div>

                            <div class="py-2">
                                <div class="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                                    <a href="#" class="w-full sm:w-auto inline-block bg-[#812755] text-white text-center px-8 py-2 rounded-full text-lg font-bold hover:bg-[#812755] transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#812755] focus:ring-opacity-50">
                                        Visionner une démo
                                    </a>
                                    <a href="#" class="w-full sm:w-auto inline-block bg-transparent border-2 border-[#812755] text-[#812755] text-center px-8 py-2 rounded-full text-lg font-bold hover:bg-[#812755] hover:text-white transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#812755] focus:ring-opacity-50">
                                        Contactez-nous
                                    </a>
                                </div>
                            </div>
                        </div>
                </div>
            </main>
        </div>
    </div>
</div>
</div>